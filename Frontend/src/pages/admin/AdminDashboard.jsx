...
import { Users, Calendar, Bed, Stethoscope, HeartHandshake, Pill, Droplet, IndianRupee, ArrowRight, UserPlus, ShieldAlert, PlusCircle, CheckCircle } from 'lucide-react';
...
export const AdminDashboard = () => {
  ...
  const { totalBeds: ctxTotalBeds, occupiedBeds: ctxOccupiedBeds, availableBeds: ctxAvailableBeds } = useRooms();
  const { bloodRequests: ctxBloodRequests } = useBloodBank();
  const { summary: ctxSummary } = useBilling();
  ...
  useEffect(() => {
    const fetchDashboard = async () => {
      try {
        ...
        console.error('Failed to fetch admin dashboard:', err?.message);
      } finally {
        setLoading(false);
      }
    };
  ...
  const totalBeds = summary.totalBeds !== undefined ? summary.totalBeds : ctxTotalBeds;
  const occupiedBeds = summary.occupiedBeds !== undefined ? summary.occupiedBeds : ctxOccupiedBeds;
  const availableBeds = summary.availableBeds !== undefined ? summary.availableBeds : ctxAvailableBeds;

  const bloodRequests = dashboardData?.recentBloodRequests || ctxBloodRequests || [];
  ...
  const bedOccupancyData = dashboardData?.charts?.bedOccupancy || [
    { name: 'Occupied', value: occupiedBeds, color: '#EF4444' },
    { name: 'Available', value: availableBeds, color: '#10B981' }
  ];

  const admissionData = dashboardData?.charts?.bedAllocationsLast7Days || [];
  const revenueData = dashboardData?.charts?.revenueTrends || [];
  const activities = dashboardData?.recentActivities || [];

  const revenueDisplay = summary.totalPaid !== undefined
    ? `₹${summary.totalPaid.toLocaleString()}`
    : (ctxSummary?.totalPaid ? `₹${ctxSummary.totalPaid.toLocaleString()}` : '₹0');

  const stats = [
    { label: 'Total Patients', value: String(summary.totalPatients ?? 0), change: 'Registered patients', icon: Users, color: 'text-blue-600 bg-blue-50' },
    { label: "Today's Appointments", value: String(summary.todayAppointments ?? 0), change: `${summary.pendingAppointments ?? 0} pending`, icon: Calendar, color: 'text-indigo-600 bg-indigo-50' },
    { label: 'Available Beds', value: String(availableBeds), change: `out of ${totalBeds}`, icon: Bed, color: 'text-emerald-600 bg-emerald-50' },
    { label: 'Occupied Beds', value: String(occupiedBeds), change: `${summary.occupancyRate ?? Math.round((occupiedBeds / (totalBeds || 1)) * 100)}% occupancy`, icon: Bed, color: 'text-rose-600 bg-rose-50' },
    { label: 'Doctors on Duty', value: String(summary.activeDoctors ?? 0), change: 'Active staff', icon: Stethoscope, color: 'text-teal-600 bg-teal-50' },
    { label: 'Nurses on Duty', value: String(summary.activeNurses ?? 0), change: 'Active staff', icon: HeartHandshake, color: 'text-amber-600 bg-amber-50' },
    { label: 'Pharmacy Alerts', value: String(summary.pharmacyAlerts ?? 0), change: 'low/out stock', icon: Pill, color: 'text-pink-600 bg-pink-50' },
    {
      label: 'Blood Requests',
      value: `${summary.pendingBloodRequests ?? pendingRequests.length} Pending`,
    ...
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-blue-500 to-indigo-600 text-white rounded-2xl p-6 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-blue-100 mt-1">Hospital-wide logistics and live statistics for MediSync AI</p>
        </div>
        <div className="flex items-center gap-2">
          <span className="h-2.5 w-2.5 rounded-full bg-emerald-400 animate-ping"></span>
          <span className="text-xs font-semibold tracking-wider uppercase text-blue-50">Live Backend Active</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <Card title="Bed Allocations" subtitle="Allocations trend (last 7 days)">
              <div className="h-60">
                {admissionData.length === 0 ? (
                  <div className="h-full flex items-center justify-center text-xs text-slate-400">
                    No bed allocation data recorded in last 7 days.
                  </div>
                ) : (
                  <ResponsiveContainer width="100%" height="100%" minHeight={200}>
                    <AreaChart data={admissionData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                      ...
                      <Area type="monotone" dataKey="Allocations" stroke="#2563EB" strokeWidth={2} fillOpacity={1} fill="url(#colorAdmissions)" />
                    </AreaChart>
                  </ResponsiveContainer>
                )}
              </div>
                  ...
                  <li key={activity.id || actIdx}>
                    <div className="relative pb-8">
                      ...
                      <div className="relative flex space-x-3">
                        <div>
                          <span className={`h-8 w-8 rounded-full flex items-center justify-center ring-8 ring-white ${activity.iconBg || (activity.type === 'success' ? 'bg-emerald-50 text-emerald-600' : activity.type === 'warning' ? 'bg-amber-50 text-amber-600' : 'bg-blue-50 text-blue-600')}`}>
                        ...
                        <div className="flex-1 min-w-0 pt-1.5 flex justify-between space-x-4">
                          <div>
                            <p className="text-xs text-slate-600">{activity.text || activity.title || activity.message}</p>
                          </div>
